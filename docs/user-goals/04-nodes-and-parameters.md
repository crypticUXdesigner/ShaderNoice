...
- **See live values when connected or driven** — Effective (live) value visible on the port or control when a graph wire, audio driver, or evaluable animation lane is active. **Audio** and **MIDI** drivers on **knob** parameters also show an **outer target arc** for that port’s **Out** range; other driven float controls use a compact target-range row ([12-parameter-drivers.md](./12-parameter-drivers.md)). When animation is evaluable, it follows transport for the **entire** timeline (lead-in, regions, gaps, tail—not only inside region rectangles) unless a connection applies on top per input mode; a timeline cue on the parameter row indicates animation is authoritative for the base value.
