...
- **Read help** — “Help” control (e.g. top bar) is available when **zero or one** node is selected:
  ...
  In node-guide mode, content appears in the callout/popover: description, optional **hook** callout, inputs, outputs, parameters (when the node body exposes controls), examples, and related/suggested links; user can close (button or click outside). Help content is keyed by node type (e.g. `node:hexagonal-grid`); sourced from static files (`node-documentation.json`, `contextual-help.json`) or node spec; missing content shows fallback or empty.
- **See the hook callout** — When a node guide defines a **`hook`** (one actionable sentence in `node-documentation.json`), the Guide shows a static info panel below the headline (sys-warn–inspired styling, **`[HINT]`** tag, no typewriter). Use it for the first wiring step or “try this next” without reading the full body.
- **Jump between node guides** — **Related** and **suggested** chips in the Guide are clickable buttons; choosing one swaps the callout to that node’s guide in place (same as selecting that node and opening Help). Keyboard focus and aria-labels support discovery without leaving the help panel.
- **Use the node context menu** — Right-click node: “Rename,” “Duplicate,” “Read Guide,” “Copy node name,” “Reset,” “Remove”; menu at pointer or near node. “Rename” selects the node and opens inline label edit (same as double-clicking the header label). “Duplicate” copies that node with a small offset (same behavior as Ctrl/Cmd+D for a single selection). “Read Guide” opens same help content as Help button. “Copy node name” copies node type id (e.g. `hexagonal-grid`) to clipboard. “Reset” restores parameters to type defaults. “Remove” removes node and all its connections; graph and runtime updated. Menu implemented as dropdown; does not block canvas after close.
...
- **Understand what can connect** — Port type pills or “compatible types” in help can guide graph connections; parameter ports may also link to [parameter drivers](./12-parameter-drivers.md) (audio, animation, planned MIDI envelope).
