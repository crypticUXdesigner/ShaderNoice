...
- **URL:** `?renderBackend=auto|webgpu|webgl` — parsed in `src/lib/App.svelte` (`parseUrlRenderBackendOverride`). Chooses the preview backend and the **same** exclusive raster API for image/video export (`RuntimeManager.getExportRasterBackend` → `runImageExport` / `runVideoExport`). A WebGPU session does not silently finish export on WebGL in one job (clear error; switch to `webgl` and reload to export on WebGL). Lib mounts export dialogs (`src/lib/app/*ExportDialogHost`); export packages stay pure run APIs.
