  ...
  for (int x = -1; x <= 1; x++) {
    for (int y = -1; y <= 1; y++) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        float influence = particleInfluence(p, worldPos, $param.particleSize, $param.particleIntensity);
        value += influence;
      }
    }
  }
...
`,
  ...
  float particles = particleSystem($input.in * $param.particleScale, particleTime, $param.particleCellSize, $param.particleCount);
  $output.out += particles * $param.particleIntensity;
`
};
