    ...
    try {
      ...
      const cache = buildFullAnalysisCache({
        ...
        onPartialCache: (partial) => {
          if (isCanceled(buildId, fileId)) return false;
          const payload: AudioAnalysisWorkerPartialResult = {
            type: 'partialResult',
            buildId,
            fileId,
            cache: partial,
          };
          // Copy was allocated in build core; transfer the buffer so main can adopt without cloning again.
          postMessage(payload, [partial.values.buffer]);
          return true;
        },
      });
