 ...
 *
 * **Policy (v1):** monophonic last-note-wins — at transport time `t`, the binding listens
 * to the most recent note-on (among filtered tracks) with `startSeconds <= t`. A newer note
 * retriggers attack even when a previous note is still held.
 *
...
export function evaluateMidiEnvelopePresetLevelAtTime(
  ...
  const active = findActiveNoteInSortedFilteredNotes(filteredNotes, transportTime);
  if (!active) return SILENT_PRESET_LEVEL;

  return computePresetLevelFromActiveNote(envelope, active, transportTime);
}
...
export function evaluateMidiEnvelopeLevelForPresetAtTime(
  ...
  return evaluateMidiEnvelopePresetLevelAtTime(filtered, envelope, transportTime).shape;
}
...
/**
 * Apply remapper output range to a preset-level shape, including velocity-scaled outMax.
 */
export function remapMidiEnvelopeBindingOutput(
  ...
  return remapValue(presetLevel.shape, 0, 1, outMin, effectiveOutMax);
}
...
export function evaluateMidiEnvelopeBindingAtTime(
  ...
  const level =
    presetLevel ??
    evaluateMidiEnvelopePresetLevelAtTime(filteredNotes, binding.envelope, transportTime);

  return remapMidiEnvelopeBindingOutput(
    level,
    binding.envelope,
    binding.envelope.outMin,
    binding.envelope.outMax
  );
}
...
export function evaluateMidiEnvelopeAtTime(
  ...
  return evaluateMidiEnvelopeBindingAtTime(binding, filtered, transportTime);
}
...
export function getMidiEnvelopeValueForParam(
  ...
  return evaluateMidiEnvelopeAtTime(snapshot, resolved, transportTime);
}
